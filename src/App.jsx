      ...
      <div style={{ maxWidth: "75%", position: "relative" }}>
        ...
        {msg.role === "assistant" && (
          <button onClick={copy} style={{
            ...
            color: copied ? "#A8FF3E" : "#3A3A5A", letterSpacing: "0.05em", padding: "2px 6px",
...
export default function SonaAgent() {
  ...
  const [allMessages, setAllMessages] = useState(() => {
    try {
      const saved = localStorage.getItem("sona_messages");
      return saved ? JSON.parse(saved) : DEFAULT_ALL_MESSAGES;
    } catch { return DEFAULT_ALL_MESSAGES; }
  });

  const [allHistory, setAllHistory] = useState(() => {
    try {
      const saved = localStorage.getItem("sona_history");
      return saved ? JSON.parse(saved) : DEFAULT_ALL_HISTORY;
    } catch { return DEFAULT_ALL_HISTORY; }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem("sona_messages", JSON.stringify(allMessages)); } catch {}
  }, [allMessages]);

  useEffect(() => {
    try { localStorage.setItem("sona_history", JSON.stringify(allHistory)); } catch {}
  }, [allHistory]);
  ...
  const saveEvent = (newEvent) => {
    ...
    const welcomeMsg = {
      role: "assistant",
      content: `Event saved! I now know everything about ${newEvent.name || "your event"}${newEvent.date ? ` on ${newEvent.date}` : ""}${newEvent.venue ? ` at ${newEvent.venue}` : ""}.\n\nEvery response from here will be tailored to your actual event. What do you need first?`,
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        textarea, input { background: transparent; border: none; outline: none; color: #E8E8F0; font-family: 'DM Sans', sans-serif; font-size: 14px; resize: none; width: 100%; line-height: 1.5; }
      ...
      <div style={{ display: "flex", height: "100vh", background: "#07070F", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ width: "260px", flexShrink: 0, borderRight: "1px solid #0F0F1A", display: "flex", flexDirection: "column", padding: "24px 16px", gap: "8px", overflowY: "auto" }}>
          ...
          {WORKSTREAMS.map((ws) => (
            <button key={ws.id} className={`ws-btn ${activeWorkstream === ws.id ? "active" : ""}`} onClick={() => setActiveWorkstream(ws.id)} style={{ borderColor: activeWorkstream === ws.id ? ws.color + "44" : undefined }}>
